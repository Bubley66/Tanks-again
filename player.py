...
from conf import HEIGHT, WIDTH, WHITE
...
class Player:
    ...
    def draw(self, sur):
        ...
        pygame.draw.line(sur, self.color, (int(c.real), int(c.imag)), (int(d.real), int(d.imag)), 10)

        v = -50-(self.radius + 40)*1j     # Vector (-50,-(r+40))
        healthbar_pos = self.pos + v

        pygame.draw.rect(sur, WHITE, (int(healthbar_pos.real), int(healthbar_pos.imag), 100, 20))
        pygame.draw.rect(sur, (51, 255, 51), (int(healthbar_pos.real), int(healthbar_pos.imag), self.health,20))
    ...
    # rewrite " if self.bullet is not None " to bullet :3
    ...
    def update(self, other_player):
        ...
        if self.bullet is not None:
            if self.bullet.shots_fired(other_player):
